...
class NeuralRouter:
    ...
    def _needs_simplification(self, state: ConversationState, user_message: str) -> bool:
        ...
        if response_policy.get("response_mode") in {"explain", "pricing_answer"}:
            return True
        if bool(neural_state.get("needs_simplification", False)) or _clean_text(neural_state.get("clarity_note", "")):
            return True
        if _clean_text(counterparty.get("clarity_need", "")) in {"simple_explanation", "practical_example", "structure"}:
            return True
        return bool(str(user_message or "").strip().endswith("?")) and state.stage_id in _LATE_DECISION_STAGES
