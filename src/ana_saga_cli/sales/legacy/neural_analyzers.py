...
from ana_saga_cli.sales.legacy.neural_router import NeuralRoute
    ...
    def _run_feynman(self, state: ConversationState, user_message: str) -> dict[str, Any]:
        ...
        user_input = self._append_extra_context(
            ...
            [
                ...
                f"- foco_de_clareza={response_policy.get('question_variable', '') or response_policy.get('main_intention', '')}",
        ...
        self.llm.annotate_last_call(
            parsed_output=payload,
            output_used=result,
            consumed_by=["state.neural_state", "prompting", "conversation_policy_engine"],
        )
        return result
